        ...
        if (!!req.body.events.length) {
          ...
          for (i = 0; i <= all_events.length - 1; i++) {
            ...
            if (processedResourceGids.has(current_asana_taskGid)) {
              console.log(`Skipping duplicate event with resource.gid: ${current_asana_taskGid}`);
            ...
            if (!occurence && current_parent_resource === "project") {
              ...
              let requestData = await createReqData(taskDetails, current_tt_userGid);
              console.log(`Executed ${i}st time`);
              let response = await postTask(requestData, timetaskSecret[current_userName]);
              ...
              const docRef = db.collection("global_task_manager").doc(current_asana_taskGid);
              await docRef.set({
                ...requestData
              });
            }

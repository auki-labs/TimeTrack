...
const {UPDATEMAP, getTaskDetails, checkOccurence,USER_MAP,USER_NAME,log } = require('../constants/constants');
      ...
      if (calculatedSignature === xHookSignature) {
        ...
        if (!!req.body.events.length) {
          ...
          if (occurence) {
            ...
            console.log("=============Update Trigger By User "+current_userName+" ============");
            console.log("Task Updated")
            var changeField = current_event.change.field;
            let taskDetails = await getTaskDetails(current_asana_taskGid,asanaSecret[current_userName]);
            console.log(taskDetails,"taskDetails");
            let updateReq = createReqData(changeField,taskDetails)  
            current_TT_taskID = await getTTID(current_asana_taskGid);
            putTask(updateReq,current_TT_taskID,timetaskSecret[current_userName])
          }
          }
        }
      }
...
async function getTTID(current_asana_taskid) {
  try {
    const docRef = db.collection('global_task_manager').doc(current_asana_taskid);

...
exports.updateTime = async (req, res) => {
  ...
  if (req.header("X-Hook-Secret")) {
    ...
    const xHookSecrets = JSON.parse(fs.readFileSync(
      path.join(__dirname, "..", "constants", "sectionTimeSecret.json"),
      "utf8"
    ));
      ...
      if (calculatedSignature === xHookSignature) {
        ...
        if (!!req.body.events.length) {
          ...
          for (i = 0; i <= all_events.length - 1; i++) {
            ...
            if (occurence) {
              console.log("=== Time Update Trigger By User " + current_userName + " ===");
              var resource = req.body.events[i].resource
              var resource_subtype = resource.resource_subtype;
              const { current_tt_taskID, current_tt_worktypeID } = await fetchDetails(current_asana_taskGid);
              var timeSheet = await getTimesheet(current_asana_taskGid, ASANA_TOKEN[current_userName]);
              if (resource_subtype == "time_tracking_entry_added" || resource_subtype == "time_tracking_entry_removed") {
                ...
                for (let i = 0; i < timeSheet.data.length; i++) {
                  ...
                  await addTimeEntry(current_asana_taskGid, currentGid, requestData, TT_TOKEN[current_userName]);
                }
                await verifyAndDeleteGids(current_asana_taskGid, timeSheet.data, current_userName)
              }else{
                console.log("CONStime_tracking_entry_changed;")
                await getCommentIdsForGids(timeSheet.data,current_asana_taskGid,current_tt_worktypeID,TT_TOKEN[current_userName])
              }
            }
          }
        }
...
async function getTimesheet(id, token) {
  const response = await axios.get(`https://app.asana.com/api/1.0/tasks/${id}/time_tracking_entries`, {
    headers: {
      Authorization: token,
    },
  });
...
async function fetchDetails(documentId) {
  try {
    ...
    if (doc.exists) {
      const data = doc.data();
      return { current_tt_taskID: data.current_tt_taskID, current_tt_worktypeID: data.current_tt_worktypeID };
    } else {
...
async function verifyAndDeleteGids(current_asana_taskGid, timeSheet, current_userName) {
  const collectionRef = db.collection('global_task_manager');
  ...
  try {
    ...
    const timeEntries = docSnapshot.exists ? docSnapshot.data().timeEntries : {};
...
async function getCommentIdsForGids(data,current_asana_taskGid,current_tt_worktypeID,token) {

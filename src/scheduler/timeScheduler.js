...
async function getTime(user, token) {
  ...
  for (let id of documentIds) {
    ...
    for (let i = 0; i < time_tracking_entryArray.length; i++) {
      ...
      await addGidsToFirestore("global_task_manager", id, currentEntry, requestData, TT_TOKEN[TT_USER]);
    }
    await checkGidsInFirestore("global_task_manager", time_tracking_entryArray, id, TT_TOKEN[TT_USER]);
  }
}
...
async function addGidsToFirestore(collectionName, taskID, timeEntry, data, token) {
...
async function checkGidsInFirestore(collectionName, time_tracking_entryArray, id,token) {
  try {
    ...
    if (docSnapshot.exists) {
      ...
      Object.entries(data).forEach(([gid, value]) => {
        const found = time_tracking_entryArray.some((entry) => entry.gid === gid);
      ...
      console.log(`Document with ID '${id}' does not exist in collection '${collectionName}'.`);
    }
  } catch (error) {

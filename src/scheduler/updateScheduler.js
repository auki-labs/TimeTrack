...
async function getStoryCount(asana_taskId, token) {
  try {
    const response = await axios.get(`https://app.asana.com/api/1.0/tasks/${asana_taskId}/stories`, {
      headers: {
        Authorization: token,
      },
    });
    ...
    for (let i = 0; i < Stories.length; i++) {
      ...
      if (resource_subtype === 'assigned' || resource_subtype === 'unassigned' || resource_subtype === 'due_date_changed' || resource_subtype === 'assigned' || resource_subtype === 'assigned') {
        counter++;
      }
    }
    return counter;
